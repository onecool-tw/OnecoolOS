...
- 每檔候選股增加一行「財報品質：標籤＋主要原因」，只讀取
  `financial_quality.label` 與 `financial_quality.reason`，並保留其財報期間。
- 這是參考財報閱讀觀念的 Onecool 初步檢核，不是怪老子的原創評分法。
- 第一版只有已公布損益表，本業未獲利但稅前獲利為正、業外淨收益高於
  本業、或稅前未獲利時標為「需留意」。業外不等於一次性收益，須另核對附註。
- 現金流、ROE槓桿及存貨／應收帳款同期比較尚未接齊；未出現上述警示
  仍顯示「資料不足」，不得自行升為「正常」。金融業需專屬檢核。
- 原始損益數字按來源累計期間使用，不冒充單季；出表日期不是財報首次公告日。
- 只作研究提醒，不改分數、名單、CTA、市場壓力、行動或資料健康判定。
- 舊快取缺少 financial_quality 時顯示「資料不足：尚無財報品質資料」。
