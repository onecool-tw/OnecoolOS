"""Validation helpers for Scenario Engine."""
...
class ScenarioError(OnecoolOSError):
    """Raised for scenario validation errors."""
...
def optional_text(value: Any, field_name: str) -> str | None:
    ...
    if not isinstance(value, str):
        raise ScenarioError(f"{field_name} must be a string.")
    value = value.strip()
...
def parse_enum(
    ...
    try:
        ...
        raise ScenarioError(f"Invalid {field_name}: {value}") from exc
...
def parse_probability(value: Any) -> Decimal | None:
    """Parse an optional probability between 0 and 1."""
    ...
    try:
        ...
        raise ScenarioError(f"Invalid probability: {value}") from exc
    if not probability.is_finite():
        raise ScenarioError(f"Invalid probability: {value}")
    if probability < Decimal("0") or probability > Decimal("1"):
        raise ScenarioError("probability must be between 0 and 1.")
    return probability
...
def parse_optional_datetime(value: Any, field_name: str) -> datetime | None:
    ...
    if isinstance(value, datetime):
        return value
    if not isinstance(value, str):
        raise ScenarioError(f"Invalid {field_name}: {value}")
    try:
...
def parse_optional_dict(value: Any, field_name: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ScenarioError(f"{field_name} must be a dictionary.")
    return dict(value)
